...
app.post("/api/pos/orders", authMiddleware, async (req, res) => {
  const { items, paymentMethod, totalAmount, orderType, deliveryRoom, notes } = req.body;
  ...
  const order = await POSOrder.create({
    ...
    notes,
    deliveryRoom: orderType === "Room Service" ? deliveryRoom : undefined,
...
app.get("/api/pos/orders", authMiddleware, async (req, res) => {
  const orders = await POSOrder.find().sort({ orderTime: -1 }).lean();
  res.json(orders);
});
...
app.get("/api/pos/orders/:id/receipt", authMiddleware, async (req, res) => {
  ...
  const lines = [];
  lines.push("Hermoso Descanso");
  lines.push("123 Hotel Street, City, Country");
  lines.push(`Order ID: ${order.orderId}`);
  lines.push(`Order Type: ${order.orderType}`);
  if (order.deliveryRoom) lines.push(`Room: ${order.deliveryRoom}`);
  lines.push(`Payment: ${order.paymentMethod}`);
  if (order.notes) lines.push(`Notes: ${order.notes}`);
  lines.push("----------------------------------------");
  order.items.forEach((item) => {
    lines.push(`${item.quantity}x ${item.dish} - $${item.subtotal.toFixed(2)}`);
  });
  lines.push("----------------------------------------");
...
app.post("/api/attendance/clock-in", authMiddleware, async (req, res) => {
  const existing = await ClockInOut.findOne({ username: req.user.id, clockOutTime: null });
  if (existing) {
    return res.status(400).json({ message: "Already clocked in" });
  }
  const clockIn = await ClockInOut.create({ username: req.user.id, role: req.user.role, clockInTime: new Date() });
  res.status(201).json(clockIn);
});

app.post("/api/attendance/clock-out", authMiddleware, async (req, res) => {
  const clockIn = await ClockInOut.findOne({ username: req.user.id, clockOutTime: null });
  if (!clockIn) {
...
app.get("/api/attendance/clock-status", authMiddleware, async (req, res) => {
  const current = await ClockInOut.findOne({ username: req.user.id, clockOutTime: null });
